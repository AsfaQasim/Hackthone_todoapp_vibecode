...
def get_task(
    ...
    try:
        ...
        from sqlalchemy import text
        query_text = text("SELECT * FROM task WHERE id = :task_id AND user_id = :user_id")
...
def update_task(
    ...
    try:
        ...
        check_query = text("SELECT * FROM task WHERE id = :task_id AND user_id = :user_id")
        ...
        update_query = text(f"UPDATE task SET {', '.join(set_parts)} WHERE id = :task_id AND user_id = :user_id")
...
def delete_task(
    ...
    try:
        ...
        # Delete the task
        delete_query = text("DELETE FROM task WHERE id = :task_id AND user_id = :user_id")
